...
public partial class Thanks : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
            ...
            // Session["pdl"] = "";
              //guid = "9C32F1BF-69F5-4DA4-A9C7-7DF50B13E7D1";
            // Session["lang"] = "FR";
               ...
               // check if it is a test order
                try
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(DATABASE_CONNECTION))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("select * from AIDOrders where AIDGUID = @guid", conn))
                        {
                            ...
                            conn.Close();
                            Session["AIDid"] = dataSet.Tables[0].Rows[0]["id"].ToString();
                ...
                else
                {
                    ...
                    if (Session["eidcomp"] == null)
                    {
                        ...
                        {
                           // SendEmail("athangaraj@isbc.ca", "MIDEID", "MIDEID.aspx?guid=" + guid + "&lan=" + lan);
                            // Server.Transfer("MIDEID.aspx?guid=" + guid + "&lan=" + lan);
                            Response.Redirect("https://infosearchsite.com/MID/MIDEID.aspx?guid=" + guid + "&lan=" + lan);
                        }
                    }
            ...
            if (uplFileUploader.HasFile && uplFileUploader2.HasFile)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            pass.Visible = true;
                           // SendEmail("athangaraj@isbc.ca", "bp1", "bp1");
                            if(Session["companyid"].ToString() == "3698")
                            {
                                //https://outlook.office365.com/owa/calendar/ISBGLOBALEBS@afimacmerged.onmicrosoft.com/bookings/
                                lblUpload.Text = "Thank You! The images have been uploaded successfully. To complete your application, you will need to prove your identity through Video Identity Validation(VID). This process is conducted by the Insurance Search Bureau of Canada (ISB) and should take approximately 5 - 7 minutes. You need to set up a video call appointment to complete your background screening.<br/><br/>  <a href=\"https://outlook.office365.com/owa/calendar/ISBGLOBALEBS@afimacmerged.onmicrosoft.com/bookings/\"> <button type=\"button\">Please click here to schedule your video call.</button></a> <br/><br/>If you encounter any problems, please don’t hesitate to contact ISB for assistance at  support@isbglobalservices.com. <br/><br/> ";
                            }
                            else
                            {
    ...
    protected void saveFileToDb(byte[] bytUpfile, byte[] bytUpfile2, string guid)
    {
        ...
        string strInsertStmt = string.Format("insert into AIDUploadImage_new values (@GUID,@TestFile,@TestFile2,0,@AIDid, getdate())");
        ...
        using (SqlConnection conn = new SqlConnection(testDbConnection))
        {
            ...
            cmdCommand.Parameters.AddWithValue("@AIDid", Session["AIDId"].ToString());
            cmdCommand.Connection.Open();
            cmdCommand.ExecuteNonQuery();
        }
       // SendEmail("athangaraj@isbc.ca", "bp2", "bp2");
    }
